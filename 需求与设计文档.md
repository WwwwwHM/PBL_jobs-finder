...
| F-RESUME-13 | AI基于原简历、当前优化稿、目标岗位和补充履历生成新版稿，不得虚构经历、职责或数据 | P0 |
| F-RESUME-14 | 生成失败时保留补充内容并允许重试；同一次诊断内生成新版稿和导出Word不重复扣配额 | P0 |
| F-RESUME-15 | 新版稿生成后仍可编辑，用户确认最终内容后再导出Word | P0 |
...
- 一次诊断及其后续新版稿生成、编辑和Word导出合计只消耗1次配额
...
def generate_resume_with_supplement(
    ...
    """
    在当前优化稿基础上融合用户补充履历，返回可编辑的新版简历文本。
...
resume_optimize:
  ...
  user_template: |
    ...
    请整合原始简历与补充履历，生成完整新版简历。经历描述使用STAR法则：
    1. 情境(Situation)：项目背景
    2. 任务(Task)：你的职责
    3. 行动(Action)：具体做了哪些事
